//2024/07/06 kasumi
//gawk "BEGIN{k=$2}{if(NF==15){k=$2}if(NF==4&&$1==0){print k}}END{print k}" T:\NINJA\E71a\work\ECC1\Muon_analysis\03_vertex_location\re_connection\event_water_fin.all > list_ecc1.txt
//I:\NINJA\E71a\work\kasumi\ECC\MuonAnalysis\x64\Release\sf_event_only.exe R:\Analysis\NINJA\E71a\5\Shifter1\acryltrack\Timestamp\muon_ecc5_including_multi.txt  list_ecc1.txt sf1.txt

//T:\NINJA\E71a\work\kasumi\sf_unique_mu_check\sf_eventonly>gawk "BEGIN{k=$2}{if(NF==15){k=$2}if(NF==4&&$1==0){print k}}END{print k}" K:\NINJA\E71a\work\suzuki\muon_analysis\03_vertex_location\re_connection\event_water_fin.all > list_ecc5.txt
//T:\NINJA\E71a\work\kasumi\sf_unique_mu_check\sf_eventonly > I:\NINJA\E71a\work\kasumi\ECC\MuonAnalysis\x64\Release\sf_event_only.exe R : \Analysis\NINJA\E71a\5\Shifter5\acryltrack\Timestamp\muon_ecc5_including_multi.txt  list_ecc5.txt sf5.txt
...
void set_event_id_list(std::string input, std::vector<int>& list);
void find_same_muon_prediction(std::string input, std::string output, std::vector<int>& list);
...
int main(int argc, char** argv) {
	...
	std::vector<int> list;
	set_event_id_list(in2,list);
	find_same_muon_prediction(input, output,list);
...
void set_event_id_list(std::string input, std::vector<int>& list) {
	...
	//	list.push_back(std::stoi(str_v[0]));
	//}
	std::cout << " Event " << count << " lines" << std::endl;
...
void find_same_muon_prediction(std::string input, std::string output,  std::vector<int>& list)
{
	...
	while (std::getline(ifs, str)) {
		...
		//if (list.find(sf.eventid) != list.end()) {
		//	ofs << str << std::endl;
		//}
		//std::cout << sf.eventid << std::endl;
