...
std::vector < std::pair<Sharing_file::Sharing_file, std::pair<int, int>>> pl4_track_multidelete(std::vector<Sharing_file::Sharing_file>& sf, std::string file_in_ECC, std::vector<output_format_link>& link) {
	...
	for (auto& b : base_pl3) {
		...
		//if (b.rawid == 2942486) {
		//	//3 2942486 7 511860664 7 577060113
		//	std::cout << b.m[0].zone << " " << b.m[0].row << " " << b.m[1].
		//}
	}
	for (auto& b : base_pl4) {
	...
	//2025/04/19
	//add for ecc7
	std::ofstream ofs("basetracks_which_did_not find_in_sel.cor.vxx.txt");
	//
	...
	for (auto itr = sf.begin(); itr != sf.end(); itr++) {
		if (itr->pl == 3) {
			...
			if (res == base_pl3_id.end()) {
				...
				//exit(1);
				ofs << "3 " << itr->ecc_id <<" "<< itr->zone[0] << " " << itr->rawid[0] << " " << itr->zone[1] << " " << itr->rawid[1] << std::endl;
				//
			}
			...
			if (res == base_pl4_id.end()) {
				fprintf(stderr, "PL%03d %d %d %d %d not found\n", itr->pl, itr->zone[0], itr->rawid[0], itr->zone[1], itr->rawid[1]);
				//exit(1);
				ofs << "4 " << itr->ecc_id << " " << itr->zone[0] << " " << itr->rawid[0] << " " << itr->zone[1] << " " << itr->rawid[1] << std::endl;
				//
			}
			if (linklet_id.count(res->second) == 0) {
				ret.push_back(std::make_pair(*itr, res->second));
				continue;
			}
			...
			for (auto res2 = range.first; res2 != range.second; res2++) {
				...
				for (auto itr2 = sf.begin(); itr2 != sf.end(); itr2++) {
					...
					if (res_pl3 == base_pl3_id.end()) {
						...
						//exit(1);
						//exit(1);
						ofs << "PL " << itr2->pl << " " << itr2->zone[0] << " " << itr2->rawid[0] << " " << itr2->zone[1] << " " << itr2->rawid[1] << std::endl;
						//

					}
					if (res_pl3->second == res2->second) {
						sf_tmp = *itr2;
						sf_detect = true;
						break;
					}
