...
void output_file(std::string filename, std::vector<Momentum_recon::Event_information>& mom, std::vector<output_format>& out, int cutpl);
...
void cut_upstream(std::vector<Momentum_recon::Event_information>& mom, double thr_ang, double thr_vph, double thr_md);
...
int main(int argc, char** argv) {
	if (argc != 5 && argc != 3 && argc != 9) {
		fprintf(stderr, "usage1:file-in-momch file-out-momch out_momch\n");
		fprintf(stderr, "usage2:file-in-momch.momch file-out-momch.txt file-out-momch.txt out_momch.momch\n");
		fprintf(stderr, "usage3:file-in-momch.momch file-out-momch.txt file-out-momch.txt out_momch.momch chi2_rat+chi2_lat-thr chi2_vph-thr MD-thr check_pl\n");
		exit(1);
	}

	std::string file_in_momch = argv[1];
	std::string file_out_momch = argv[2];
	std::string file_out_best_pl_momch = "";
	std::string file_cut_momch = "";
	...
	if (argc == 5) {
		file_out_best_pl_momch = argv[3];
		file_cut_momch = argv[4];
	}
	if (argc == 9) {
		file_out_best_pl_momch = argv[3];
		file_cut_momch = argv[4];
	...
	if (argc != 5) {
		output_file(file_out_momch, momch, out, cutpl);
	}

	if (argc == 5) {
		output_file_all(file_out_momch, momch, out);

		file_out_best_pl_momch = argv[3];
		file_cut_momch = argv[4];
		output_file_best(file_out_best_pl_momch, out);

		cut_upstream(momch,thr_ChiAng,thr_ChiVph,thr_MD);//chi2Ç™thrÇÊÇËçÇÇ¢Ç‡ÇÃÇÕçÌèú
...
void output_file(std::string filename, std::vector<Momentum_recon::Event_information>& mom, std::vector<output_format>& out, int cutpl) {
	...
	for (auto& ev : mom) {
		...
		for (auto& c : ev.chains) {
			...
			for (int i = 1; i < cutpl; i++) {// && i <= 2; i++) {

				if (pl_v[i] < 20)continue;
...
void cut_upstream(std::vector<Momentum_recon::Event_information>& mom, double thr_ang, double thr_vph, double thr_md) {
	std::ofstream ofs("chain_cut_list.txt");
	...
	for (auto& ev : mom) {
		for (auto& c : ev.chains) {
			...
			for (int i = 1; i < pl_v.size() && i <= 8; i++) {
				...
				out_tmp.chi2[2] = chi2[2];
				out_tmp.chi2[3] = chi2[3];
				if (chi2[0] + chi2[1] > thr_ang || chi2[2] > thr_vph || md > thr_md) {
					if (chi2[0] + chi2[1] > thr_ang) {
						cous[0] = 1;
					}
					if (chi2[2] > thr_vph) {
						cous[1] = 1;
					...
					printf("eventid %d\n", ev.groupid);
					ofs << std::setw(8) << ev.groupid << std::setw(4) << pl
						<< std::setw(3) << cous[0] << std::setw(3) << cous[1] << std::setw(3) << cous[2] << std::endl;
					flg++;
