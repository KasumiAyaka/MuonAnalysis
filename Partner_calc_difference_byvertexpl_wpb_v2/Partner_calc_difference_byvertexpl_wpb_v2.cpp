...
#include <filesystem>
#include <omp.h>
//void add_mom:groupid skip

class Difference {
public:
	int groupid, chainid, vertex_pl, target_pl[2], direction, add_nseg, edge_pl[2], penetrate_flg;
	double ax, ay, dax, day, dx, dy, dar, dal, dr, dl;
	double  s_dar, s_dal, s_dr, s_dl, s_pb_angle, s_pb_position;
	double mcs_pb, mcs_pb_error[2];
};
class Sigma_list {
public:
	int peke;
...
void add_mom(std::vector<Momentum_recon::Event_information>& all, std::map<int, std::map<int, Momentum_recon::Mom_chain>>& momch_mom_map);
std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>> peke_pair_pickup(std::vector<Momentum_recon::Event_information>& momch_ev);
void Set_pair_information(std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>>& pair_map, std::string file_in_ECC, std::string file_in_base);
std::vector<Difference> Calc_difference(std::vector<Momentum_recon::Event_information>& momch_ev, std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>>& pair_map);
...
std::vector<Momentum_recon::Event_information > cut_penetrate(std::vector<Momentum_recon::Event_information >& momch_ev, std::map<int, std::map<int, Momentum_recon::Mom_chain>>& momch_mom_map, std::set<std::pair<int, int>>& id_list);
...
int main(int argc, char** argv) {
	if (argc != 9) {
		printf("argc %d\n", argc);
		fprintf(stderr, "usage:file-in-momch-all file-in-momch-mom sigma_list_water sigma_list_iron file-in-ECC file_in_base file-out file_out_momch\n");
		exit(1);
	}
	...
	std::string file_in_base = argv[6];
	...
	//周辺のpairをpickup
	std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>>peke_pair = peke_pair_pickup(momch_all);
	//localなpair情報を与える
	Set_pair_information(peke_pair, file_in_ECC, file_in_base);

	//peke pairを使った実装
	std::vector<Difference>diff = Calc_difference(momch_all, peke_pair);
	...
	//peke pairを使った実装
	momch_all = cut_penetrate(momch_all, momch_mom_map, penetrate_id);
	...
	for (auto& ev : all) {
		...
		for (auto& c : ev.chains) {
			...
			if (mom_map.count(c.chainid) == 0) {
				...
				//printf("event %d chain %d     %.1lf\n", ev.groupid, c.chainid, mom.ecc_mcs_mom[0]);
			}
			...
			if (mom_map_inv.count(c.chainid) == 0) {
				...
				//printf("event %d chain %d inv %.1lf\n", ev.groupid, c.chainid, mom_inv.ecc_mcs_mom[0]);
			}
			...
			if (count_pb_angle_diff(mom) < 2 && count_pb_angle_diff(mom_inv) < 2) {
				...
				c.ecc_mcs_mom[0] = mom_inv.ecc_mcs_mom[0];
				c.ecc_mcs_mom[1] = mom_inv.ecc_mcs_mom[1];
				c.ecc_mcs_mom_error[0][0] = mom_inv.ecc_mcs_mom_error[0][0];
				c.ecc_mcs_mom_error[0][1] = mom_inv.ecc_mcs_mom_error[0][1];
				c.ecc_mcs_mom_error[1][0] = mom_inv.ecc_mcs_mom_error[1][0];
				c.ecc_mcs_mom_error[1][1] = mom_inv.ecc_mcs_mom_error[1][1];
			}
			else {
				c.ecc_mcs_mom[0] = mom.ecc_mcs_mom[0];
				c.ecc_mcs_mom[1] = mom.ecc_mcs_mom[1];
				c.ecc_mcs_mom_error[0][0] = mom.ecc_mcs_mom_error[0][0];
				c.ecc_mcs_mom_error[0][1] = mom.ecc_mcs_mom_error[0][1];
				c.ecc_mcs_mom_error[1][0] = mom.ecc_mcs_mom_error[1][0];
				c.ecc_mcs_mom_error[1][1] = mom.ecc_mcs_mom_error[1][1];
			}
		}
	}
}
...
std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>> peke_pair_pickup(std::vector<Momentum_recon::Event_information>& momch_ev) {
	...
	std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>> ret;
	std::pair<int, int> id;
	for (auto& ev : momch_ev) {
		...
		for (auto& c : ev.chains) {
			id.second = c.chainid;
			if (c.chainid == 0)continue;
			Momentum_recon::Mom_basetrack attach_base;
			std::vector<Momentum_recon::Mom_basetrack> target;
			...
			if (c.direction == 1) {
				...
				for (auto& b : c.base) {
					if (attach_base.pl - b.pl == 0)continue;
					if (abs(attach_base.pl - b.pl) > 3)continue;
					target.push_back(b);
				}
			}
			...
			if (target.size() > 0) {
				ret.insert(std::make_pair(id, std::make_pair(attach_base, target)));
			}
		}
	}
	return ret;
...
void Get_alignment_inv_filename(std::vector<std::pair<std::pair<int, int>, std::string> >& ret, std::string file_in_ECC) {
	//std::vector<std::pair<std::pair<int, int>, std::string> > ret;
	std::map<std::pair<int, int>, std::string> ret_map;
...
void Set_pair_information(std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>>& pair_map, std::string file_in_ECC, std::string file_in_base) {
	...
	for (auto itr = pair_map.begin(); itr != pair_map.end(); itr++) {
		for (auto itr2 = itr->second.second.begin(); itr2 != itr->second.second.end(); itr2++) {
			pl_pair.insert(std::make_pair(itr->second.first.pl, itr2->pl));
			pl_pair_to_base.insert(std::make_pair(std::make_pair(itr->second.first.pl, itr2->pl), &(*itr2)));
			...
			res = read_list.find(itr2->pl);
			if (res == read_list.end()) {
				std::multimap<int, Momentum_recon::Mom_basetrack*> base_map_p;
				base_map_p.insert(std::make_pair(itr2->rawid, &(*itr2)));
				read_list.insert(std::make_pair(itr2->pl, base_map_p));
			}
			else {
				res->second.insert(std::make_pair(itr2->rawid, &(*itr2)));
			}
		}
	}
	//basetrack情報の付与
	...
	std::vector<std::pair<std::pair<int, int>, std::string> >  files_in_align;
	files_in_align = Get_alignment_filename(file_in_ECC);
	Get_alignment_inv_filename(files_in_align, file_in_ECC);
...
void Calc_sigma_pb(Difference& diff, std::pair<Momentum_recon::Mom_basetrack, Momentum_recon::Mom_basetrack>& pair) {
	double pb;
	if (diff.mcs_pb < 0) {
		diff.s_pb_angle = 0;
...
std::vector<Difference> Calc_difference(std::vector<Momentum_recon::Event_information>& momch_ev, std::map<std::pair<int, int>, std::pair<Momentum_recon::Mom_basetrack, std::vector<Momentum_recon::Mom_basetrack>>>& pair_map) {
	...
	for (auto& ev : momch_ev) {
		...
		for (auto& c : ev.chains) {
			...
			diff.add_nseg = 0;
			diff.penetrate_flg = 0;
			diff.target_pl[0] = -1;
			diff.target_pl[1] = -1;
			diff.s_pb_angle = 0;
			diff.s_pb_position = 0;
			diff.mcs_pb = -1;
			diff.mcs_pb_error[0] = -1;
			diff.mcs_pb_error[1] = -1;

			double pb_error[2];
			diff.mcs_pb = c.Get_muon_mcs_pb();
			c.Get_muon_pb_mcs_error(pb_error);
			diff.mcs_pb_error[0] = pb_error[0];
			diff.mcs_pb_error[1] = pb_error[1];


			std::pair<Momentum_recon::Mom_basetrack, Momentum_recon::Mom_basetrack> divide_pair;
			std::vector<Momentum_recon::Mom_basetrack> base_v;
			base_v.push_back(pair_itr->second.first);
			for (auto itr = pair_itr->second.second.begin(); itr != pair_itr->second.second.end(); itr++) {
				base_v.push_back(*itr);
			}
			for (int i = 0; i < base_v.size(); i++) {
				for (int j = i + 1; j < base_v.size(); j++) {
					fprintf(stderr, "event %d chain %d partner\n", diff.groupid, c.chainid);

					diff.target_pl[0] = base_v[i].pl;
					diff.target_pl[1] = base_v[j].pl;
					auto pair_base = std::make_pair(base_v[i], base_v[j]);
					if (diff.target_pl[0] > diff.target_pl[1]) {
						std::swap(diff.target_pl[0], diff.target_pl[1]);
						std::swap(pair_base.first, pair_base.second);
					}

					Calc_difference(diff, pair_base);
					Calc_sigma_pb(diff, pair_base);
					ret.push_back(diff);
				}
			}
		}
...
void Calc_sigma(std::vector<Difference>& diff, std::map<int, std::map<double, Sigma_list>>& sigma_list_water, std::map<int, std::map<double, Sigma_list>>& sigma_list_iron) {
	...
	for (auto itr = diff.begin(); itr != diff.end(); itr++) {
		pl0 = std::min(itr->target_pl[0], itr->target_pl[1]);
		pl1 = std::max(itr->target_pl[0], itr->target_pl[1]);
		peke = pl1 - pl0 - 1;
		angle = sqrt(pow(itr->ax, 2) + pow(itr->ay, 2));
		auto sigma_peke = sigma_list_iron.find(peke);

		if (judge_pl_material(pl0, peke) == 1) {
			auto sigma_peke = sigma_list_iron.find(peke);
			if (sigma_peke == sigma_list_iron.end()) {
				fprintf(stderr, "search Iron list\n");
...
void output_difference(std::string filename, std::vector<Difference>& diff) {
	...
	for (auto itr = diff.begin(); itr != diff.end(); itr++) {
		ofs << std::right << std::fixed
			...
			<< std::setw(2) << std::setprecision(0) << itr->penetrate_flg << " "
			...
			<< std::setw(7) << std::setprecision(1) << itr->mcs_pb << " "
			<< std::setw(7) << std::setprecision(1) << itr->mcs_pb_error[0] << " "
			<< std::setw(7) << std::setprecision(1) << itr->mcs_pb_error[1] << std::endl;
	}
}


//Get_penetrate_list//
std::set<std::pair<int, int>> first_selection(std::map< std::pair<int, int>, std::vector<Difference>>& diff_map) {
	std::set<std::pair<int, int>> ret;
	for (auto itr = diff_map.begin(); itr != diff_map.end(); itr++) {
		Difference target_diff;
		//targetとなる隣接の抜出
		int count = 0;
		int n_diff = 999;
		for (auto& d : itr->second) {
			if (d.target_pl[0] <= d.vertex_pl && d.target_pl[1] > d.vertex_pl) {
				count++;
				if (n_diff > d.target_pl[1] - d.target_pl[0]) {
					n_diff = d.target_pl[1] - d.target_pl[0];
					target_diff = d;
				}
			}
		}
		if (count == 0)continue;
		double chi2 = pow(target_diff.s_dal, 2) + pow(target_diff.s_dar, 2) + pow(target_diff.s_dl, 2) + pow(target_diff.s_dr, 2);
		//printf("PL%03d PL%03d\n", target_diff.target_pl[0], target_diff.target_pl[1]);
		//printf("%.2lf\n", chi2);
		if (chi2 > 30)continue;
...
std::set<std::pair<int, int>> second_selection(std::map< std::pair<int, int>, std::vector<Difference>>& diff_map) {
	std::set<std::pair<int, int>> ret;
	for (auto itr = diff_map.begin(); itr != diff_map.end(); itr++) {
		Difference target_diff;
		//targetとなる隣接の抜出
		...
		for (auto& d : itr->second) {
			if (d.direction == 1) {
				for (int i = 0; i < 2; i++) {
					if (d.target_pl[i] > d.vertex_pl)continue;
					if (attach_pl<0 || d.target_pl[i] >attach_pl) {
						attach_pl = d.target_pl[i];
					}
				}
			}
		...
		if (attach_pl < 0)continue;
		//printf("attach PL %03d\n", attach_pl);
		for (auto& d : itr->second) {
			if (d.target_pl[0] == attach_pl)continue;
			if (d.target_pl[1] == attach_pl)continue;
			if (d.target_pl[0] <= d.vertex_pl && d.target_pl[1] > d.vertex_pl) {
				count++;
				if (n_diff > d.target_pl[1] - d.target_pl[0]) {
					n_diff = d.target_pl[1] - d.target_pl[0];
...
std::set<std::pair<int, int>> Get_penetrate_list(std::vector<Difference>& diff) {
	std::map< std::pair<int, int>, std::vector<Difference>>diff_map;
	for (auto itr = diff.begin(); itr != diff.end(); itr++) {
		std::pair<int, int> group_event = std::make_pair(itr->groupid, itr->chainid);
		if (diff_map.count(group_event) == 0) {
			std::vector<Difference> diff_v;
			diff_v.push_back(*itr);
			diff_map.insert(std::make_pair(group_event, diff_v));
		}
		else {
			diff_map.find(group_event)->second.push_back(*itr);
		}
	}

	std::set<std::pair<int, int>> condition_1 = first_selection(diff_map);
	std::set<std::pair<int, int>> condition_10 = second_selection(diff_map);

	//flg set
	for (auto itr = diff.begin(); itr != diff.end(); itr++) {
		std::pair<int, int> group_event = std::make_pair(itr->groupid, itr->chainid);
		if (condition_1.count(group_event) == 1) {
			itr->penetrate_flg += 1;
		}
	}
	for (auto itr = diff.begin(); itr != diff.end(); itr++) {
		std::pair<int, int> group_event = std::make_pair(itr->groupid, itr->chainid);
		if (condition_10.count(group_event) == 1) {
			itr->penetrate_flg += 10;
		}
	}


	std::set<std::pair<int, int>> ret;
	for (auto itr = condition_1.begin(); itr != condition_1.end(); itr++) {
		ret.insert(*itr);
	}
	for (auto itr = condition_10.begin(); itr != condition_10.end(); itr++) {
		ret.insert(*itr);
	}
	return ret;
