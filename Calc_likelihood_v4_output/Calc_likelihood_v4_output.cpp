...
int main(int argc, char** argv) {
	...
	std::string file_out_point = argv[1];
	std::string file_out = argv[2];
	double pion_thr = std::stod(argv[3]);
	...
	std::string fileame_fit = argv[5];
	std::string file_in_pion = argv[6];

	std::map<int, std::map<double, VPH_pion_mip>> pion_pdf_map = Read_pion_PDF(file_in_pion);
	...
	for (int i_ang = 0; i_ang < 20; i_ang++) {
		...
		for (double pb = 1; pb < 1500; pb += 10) {
			for (double vph = 1; vph < 300; vph += 2) {
				Chain_inf c;
	...
	std::ofstream ofs(file_out_point);
	...
	for (auto& c : c_inf) {
		...
		if (pion_pdf_map.upper_bound(int(c.angle * 10)) == pion_pdf_map.end()) {
			pion_pdf = pion_pdf_map.rbegin()->second;
		}
		else {
			pion_pdf = pion_pdf_map.upper_bound(int(c.angle * 10))->second;
		}

		//protonの確率
		c.proton_likelihood = (1 + std::erf((c.vph - vph_param.Calc_VPH_mean_proton(c.pb)) / (sqrt(2) * vph_param.Calc_VPH_sigma_proton(c.pb)))) / 2;
		//pionの確率
		c.pion_likelihood = 1 - (1 + std::erf((c.vph - vph_param.Calc_VPH_mean_pion(c.pb)) / (sqrt(2) * vph_param.Calc_VPH_sigma_pion(c.pb)))) / 2;
		if (pion_pdf.begin()->second.pb_max + 100 < c.pb) {
			c.pion_likelihood = Calc_prob(c.vph, pion_pdf);
		}

		c.likelihood_ratio = c.pion_likelihood / (c.pion_likelihood + c.proton_likelihood);

		if (c.pid == 0) {
			if (pion_pdf.begin()->second.pb_max + 100 < c.pb && pion_pdf.begin()->second.expect > c.vph) {
				c.likelihood_ratio = 0;
				c.pid = 211;
				count_particle[1]++;
			}
			else if (pion_pdf.begin()->second.pb_max + 100 >= c.pb && vph_param.Calc_VPH_mean_pion(c.pb) > c.vph) {
				c.likelihood_ratio = 0;
				c.pid = 211;
				count_particle[1]++;
			}
			else if (vph_param.Calc_VPH_mean_proton(c.pb) < c.vph) {
	...
	for (int i_ang = 0; i_ang < 20; i_ang++) {
		...
		std::map<double, VPH_pion_mip> pion_pdf;
		if (pion_pdf_map.upper_bound(int((angle_min + angle_max) / 2 * 10)) == pion_pdf_map.end()) {
			pion_pdf = pion_pdf_map.rbegin()->second;
		}
		else {
			pion_pdf = pion_pdf_map.upper_bound(int((angle_min + angle_max) / 2 * 10))->second;
		}

		for (double pb = 10; pb < 1500; pb++) {
			double proton_likelihood, pion_likelihood, likelihood_ratio, likelihood_ratio_pre;
			std::pair<double, double> l_ratio;
			double vph_pich = 1;
			double vph = 0;
			double vph_thr;
			int pid;
			while (true) {
				vph += vph_pich;
				//protonの確率
				proton_likelihood = (1 + std::erf((vph - vph_param.Calc_VPH_mean_proton(pb)) / (sqrt(2) * vph_param.Calc_VPH_sigma_proton(pb)))) / 2;
				//pionの確率
				pion_likelihood = 1 - (1 + std::erf((vph - vph_param.Calc_VPH_mean_pion(pb)) / (sqrt(2) * vph_param.Calc_VPH_sigma_pion(pb)))) / 2;
				if (pion_pdf.begin()->second.pb_max + 100 < pb) {
					pion_likelihood = Calc_prob(vph, pion_pdf);
				}
				likelihood_ratio = pion_likelihood / (pion_likelihood + proton_likelihood);


				if (pion_pdf.begin()->second.pb_max + 100 < pb && pion_pdf.begin()->second.expect > vph) {
					pid = 211;
				}
				else if (pion_pdf.begin()->second.pb_max + 100 >= pb && vph_param.Calc_VPH_mean_pion(pb) > vph) {
					pid = 211;
				}
				else if (vph_param.Calc_VPH_mean_proton(pb) < vph) {
					pid = 2212;
				}
				else if (likelihood_ratio > pion_thr) {
					pid = 211;
				}
				else {
					pid = 2212;
				...
				if (pid == 2212) {
					if (vph_param.Calc_VPH_mean_pion(pb) < vph) {
						vph_thr = vph - vph_pich * 0.5;
					}
					else {
						vph_thr = vph + vph_pich / (likelihood_ratio - likelihood_ratio_pre) * (pion_thr - likelihood_ratio);
					}
					//printf("%.1lf %.1lf %.3lf\n", vph - vph_pich, vph, vph_thr);

					break;
				}
				likelihood_ratio_pre = likelihood_ratio;
			}

			ofs << std::fixed << std::right
				<< std::setw(2) << std::setprecision(0) << i_ang << " "
				<< std::setw(4) << std::setprecision(1) << angle_min << " "
				<< std::setw(4) << std::setprecision(1) << angle_max << " "
				<< std::setw(5) << std::setprecision(1) << pb << " "
				<< std::setw(6) << std::setprecision(2) << vph_thr << std::endl;
...
double Calc_prob(double vph, std::map<double, VPH_pion_mip>pion_vph) {
	...
	for (auto itr = pion_vph.rbegin(); itr != std::next(pion_vph.rend(), -1); itr++) {
		...
		if (point[1].vph > vph) {
			...
			double prob_middle_scale = prob_middle_area / ((point[1].prob + point[0].prob) * (point[1].vph - point[0].vph) / 2) * ((point[0].prob + point[1].prob) / 2);
