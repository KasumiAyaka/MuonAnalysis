...
void output_file(std::string filename, std::vector<Momentum_recon::Event_information>& mom, std::vector<output_format>& out, std::multimap<int, int>& kink, std::multimap<int, int>& nkink);
...
void cut_upstream(std::vector<Momentum_recon::Event_information>& mom, std::multimap<int, int>& kink, std::string output);
...
int main(int argc, char** argv) {
	if (argc != 5) {
		fprintf(stderr, "usage:file-in-momch file-out-momch file-out-momch2 sigma.txt\n");
		fprintf(stderr, "usage:file-in-momch.momch file-out-momch.txt file-out-momch.txt .txt\n");
		exit(1);
	}
	...
	std::multimap<int, int> nkink;
	output_file(file_out_momch, momch, out, kink, nkink);
	//output_file_best(file_out_best_pl_momch, out);

	cut_upstream(momch,kink, file_out_best_pl_momch);
	Culc_dangle_sigma(momch, kink, file_cut_momch);
	std::string output = "test\\not_kink.txt";
	Culc_dangle_sigma_not_kink(momch, nkink, output);
		...
		for (auto& c : ev.chains) {
			...
			for (int i = 1; i < pl_v.size(); i++) {// && i <= 2; i++) {
				...
				if (chi2[0] + chi2[1] > 100 || chi2[2] > 25 || out_tmp.md > 100) {
					...
					nkink.insert(std::make_pair(kc.groupid, kc.pl));
				}
...
void Calc_divide_angle_radial(Momentum_recon::Mom_chain& c, int pl, double& dar, double& rms, double& ave, double& sd) {
	...
	for (auto itr = c.base_pair.begin(); itr != c.base_pair.end(); itr++) {//pl:small-->large
		...
		if (itr->first.pl == pl) {//pl-->kinkŽè‘O, pl+1--pl=kink
			dar = dang;
		}
		else if (itr->second.pl <= pl) {
			sum2 += dang * dang;
