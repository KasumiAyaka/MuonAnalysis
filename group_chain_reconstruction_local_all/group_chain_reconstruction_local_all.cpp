...
std::vector< Group_file> read_chain_file(std::string filename);
std::vector< Group_file> read_group_file_bin(std::string filename);

void ChainID_reroll(mfile0::Mfile& m);
void Group_pos_check(std::vector< Group_file>& group);

int main(int argc, char** argv) {
	if (argc != 4) {
		fprintf(stderr, "prg file-in-group file-in-ECC file-out-all-chain\n");
		exit(1);
	}

	std::string file_in_group = argv[1];
	std::string file_in_ECC = argv[2];
	std::string file_out_mfile_all = argv[3];
	//ここ
	...
	std::vector <  Group_file >group = read_group_file_bin(file_in_group);

	Group_pos_check(group);
	...
	for (int i = 0; i < group.size(); i++) {
		//原因不明うまくいかない
		//if (group[i].eventid <60428323)continue;
		//if (group[i].eventid == 60428323)continue;
		//if (group[i].eventid == 60430003)continue;
		//if (group[i].eventid == 60433344)continue;
		//if (group[i].eventid == 60435989)continue;

		//if (i % 10000 == 0 || i + 1 == group.size()) {
		if (i % 1000 == 0 || i + 1 == group.size()) {
	...
	ChainID_reroll(m);
	mfile1::write_mfile_extension(file_out_mfile_all, m);
...
std::vector< Group_file> read_group_file(std::string filename) {
	...
	while (ifs >> out.eventid >> out.trackid >> num_comfirmed_path >> num_cut_path >> num_select_path) {
		for (int i = 0; i < num_comfirmed_path; i++) {
			ifs >> id >> std::get<0>(path) >> std::get<1>(path) >> std::get<2>(path) >> std::get<3>(path);
			out.linklet.push_back(path);
		}
		for (int i = 0; i < num_cut_path; i++) {
			ifs >> id >> std::get<0>(path) >> std::get<1>(path) >> std::get<2>(path) >> std::get<3>(path);
			out.linklet.push_back(path);
		}
		for (int i = 0; i < num_select_path; i++) {
			ifs >> id >> std::get<0>(path) >> std::get<1>(path) >> std::get<2>(path) >> std::get<3>(path);
			out.linklet.push_back(path);
		}
		out.link_num = out.linklet.size();
		ret.push_back(out);
		out.linklet.clear();
	}
	return ret;
...
std::vector< Group_file> read_group_file_bin(std::string filename) {
	...
	int num_comfirmed_path, num_cut_path, num_select_path;
	int cnt = 0;
	while (ifs.read((char*)&out.eventid, sizeof(int))) {
		ifs.read((char*)&out.trackid, sizeof(int));
		ifs.read((char*)&num_comfirmed_path, sizeof(int));
		ifs.read((char*)&num_cut_path, sizeof(int));
		ifs.read((char*)&num_select_path, sizeof(int));
...
void Group_pos_check(std::vector< Group_file>& group) {
	...
	for (auto& g : group) {
		for (auto& l : g.linklet) {
			pl0 = std::get<0>(l) / 10;
			pl1 = std::get<1>(l) / 10;
			std::get<0>(l) = pl0 * 10 + 1;
			std::get<1>(l) = pl1 * 10 + 1;
			//printf("%3d %3d\n", pl0, pl1);
		}
	}
