...
bool operator<(const List2& lhs, const List2& rhs) {
	return std::tie(lhs.gid, lhs.k, lhs.tid) < std::tie(rhs.gid, rhs.k,rhs.tid);
}
...
int main(int argc, char** argv) {
	...
	if (!ifs) {
		std::cerr << "File open error : " << input1 << std::endl;
		exit(1);
	}

	List1 l = { 0 };
	std::cout << "now reading..." << std::endl;
	while (ifs >> l.pl >> l.pl >> l.unixtime) {//gid,pl,rid
		ret.push_back(l);
	}
	...
	List2 l2 = { 0 };
	std::cout << "now reading..." << std::endl;
	std::make_pair<int, int> key;
	while (ifs >> l.eventid >> l2.bsd_spillnum >> l2.unixtime >> l.unixtime) {//gid,pl,rid,tid 
		ret.push_back(l);
	}
	std::cout << ret.size() << std::endl;
