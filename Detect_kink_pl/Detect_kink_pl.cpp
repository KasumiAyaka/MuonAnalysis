...
int main(int argc, char** argv) {
	...
	std::string file_out_momch = argv[2];//chain dist
	std::string file_out_best_pl_momch = argv[3];//most kink pl
	std::string file_cut_momch = argv[4];//output
		...
		for (auto& c : ev.chains) {
			...
			for (int i = 1; i < pl_v.size(); i++) {
				...
				ofs << std::fixed << std::right
					...
					<<std::setw(3)<<std::setprecision(0)<<kink_flg
					<< std::endl;
...
void cut_upstream(std::vector<Momentum_recon::Event_information>& mom) {
	std::ofstream ofs("Kink_cand_list.txt");
	...
	ofs << std::fixed << std::right
		...
		<< std::setw(7) << std::setprecision(0) << "angle" << " "
		<< std::setw(4) << std::setprecision(0) << "vph" << " "
		<< std::setw(4) << std::setprecision(0) << "md" << std::endl;
			...
			for (int i = 1; i < pl_v.size(); i++) {
				...
				if (chi2[0] + chi2[1] > 100 || chi2[2] > 25 || md > 100) {
					...
					ofs << std::fixed << std::right
						<< std::setw(10) << std::setprecision(0) << ev.groupid << " "
						<< std::setw(4) << std::setprecision(0) << pl << " "
						<< std::setw(7) << std::setprecision(0) << cous[0] << " "
						<< std::setw(4) << std::setprecision(0) << cous[1] << " "
						<< std::setw(4) << std::setprecision(0) << cous[2] << " " << std::endl;
					flg++;
