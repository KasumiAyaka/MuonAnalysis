...
void set_VTXfile(std::string input, std::string output, double md_thr);
void matching(std::multimap<int, Btrk>& trk, std::set<CutList>& cut_pair, std::map<int, Key>& mu, std::string output);

int main(int argc, char** argv) {
	if (argc < 4 ) {
		fprintf(stderr, "usage1:in-vtx.txt out-list.txt ");
		exit(1);
	}
	std::string input = argv[1];
	std::string output = argv[2];
	double md_thr = std::stod(argv[3]);
	set_VTXfile(input, output, md_thr);
...
void set_VTXfile(std::string input, std::string output, double md_thr) {
	...
	std::cout << input << " ; MD < " << md_thr << "‚ğselect." << std::endl;
	int count = 0;
	CutList k;
	Btrk b;
	int gid = 0;
	...
	matching(trk, cut_pair, mu, output);
		...
		for (auto itr = list.first; itr != list.second; itr++) {
			...
			if (lst.find(k) != lst.end()) {
				...
				ofs << std::right << std::fixed
							<< std::setw(5) << std::setprecision(0) << k.gid << std::endl;
				}
			}
		}
	}
